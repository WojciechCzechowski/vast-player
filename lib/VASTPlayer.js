...
var VAST = require('./vastacular/lib/VAST');
...
const NonLinearMediaPlayer = require('./players/NonLinearMediaPlayer');
...
Object.defineProperties(VASTPlayer.prototype, {
    ...
    adType: {
        get: function () {
            return this.__private__.vast.ads[0].creatives[0].type;
        }
    },
    skipoffset: {
        get: function() {
            return this.__private__.vast.ads[0].creatives[0].skipoffset;
        }
    },
    impressionUri: {
      get: function() {
          return this.__private__.vast.ads[0].impressions[0].uri;
      }
    },
    pickedMedia: proxyProp('media'),
    adRemainingTime: proxyProp('adRemainingTime'),
    adDuration: proxyProp('adDuration'),
    adVolume: proxyProp('adVolume')
});

VASTPlayer.prototype.load = function load(uri, adContainer) {
    var self = this;
    ...
    return VAST.fetch(uri, config).then(function loadPlayer(vast) {
        var config = (function() {
            ...
            var resources = vast.filter('ads[0].creatives[0].ads', function() { return true; });
            ...
            if (jsVPAIDFiles.length > 0) {
                ...
                return {
                    player: new NonLinearMediaPlayer(self.container, adContainer),
                    mediaFiles: resources,
                }
            }
